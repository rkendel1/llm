...
export class ModelsCommand extends Command {
  ...
  async execute(context: CommandContext): Promise<CommandResult> {
    try { const snapshot = await resolveRegistry(), models = snapshot.models;
      const rows = models.slice(0, 50).map((model) => [model.id, model.routes.some((route) => route.execution === "cloud") ? "cloud" : model.routes.some((route) => route.execution === "local") ? "local" : "unknown", String(model.limits.contextWindow ?? "unknown"), symbol(model.capabilities.vision), symbol(model.capabilities.tools), `${Math.round((model.intelligence?.quality.confidence ?? model.quality.confidence) * 100)}%`, String(model.routes.length)]);
      ...
      console.log(table(["Model", "Execution", "Context", "Vision", "Tools", "Quality", "Routes"], rows)); console.log("Legend: ✓ supported  ✗ unsupported  ? unknown");
