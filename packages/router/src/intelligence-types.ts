...
export interface RouteHealth { status: "healthy" | "degraded" | "unavailable" | "unknown"; lastVerifiedAt?: string; failureRate?: number }
...
export interface IntelligentScoreBreakdown { capability: number; quality: number; freshness: number; availability: number; cost: number; policy: number; total: number }
...
export interface RoutingObservation { modelId: string; routeId: string; requestId: string; event: "success" | "timeout" | "rate_limited" | "unavailable" | "capability_failure" | "execution_failure"; observedAt: string }
